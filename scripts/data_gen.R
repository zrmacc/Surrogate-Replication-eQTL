#' Purpose: Data generation for surrogate model simulations.
#' Updated: 2021-08-03
...
GenCovar <- function(n) {
  ...
  covars <- cbind(
    "Age" = stats::rgamma(n = n, shape = 250, rate = 5),
    "Sex" = stats::rbinom(n = n, size = 1, prob = 0.5)
  )
...
#' Does *not* include the contribution of genotype.
#'
#' @param dist Distribution, from among "exp", "lognorm", "norm", "t".
...
GenPheno <- function(dist = "norm", eta, n0, n1 = 0, n2 = 0, rho) {
  ...
  if (dist == "exp") {
    e1 <- stats::rexp(n = n)
    e2 <- rho * e1 + sqrt(1 - rho^2) * stats::rexp(n = n)
    resid <- cbind(e1, e2)
  }
